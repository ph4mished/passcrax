...
// Problem: A better and robust approach will be addressed because shows output in unordered list. most likely hashed should be shown at the top and least likely, at the bottom
var unknown string
var HASH_PATTERNS = map[string]*regexp.Regexp{
	...
	"SHA-1-Base64": regexp.MustCompile(`^(?i)[a-zA-Z0-9+/]{28}={0,2}$`),
	...
	"SHA3-224":     regexp.MustCompile(`^(?i)[a-f0-9]{56}$`),
	"SHA3-256":     regexp.MustCompile(`^(?i)[a-f0-9]{64}$`),
	"SHA3-384":     regexp.MustCompile(`^(?i)[a-f0-9]{96}$`),
	"SHA3-512":     regexp.MustCompile(`^(?i)[a-f0-9]{128}$`),
	...
	"LM":   regexp.MustCompile(`^(?i)[A-F0-9]{32}$`),
...
func FindHash(targetHash string) string {
	...
	unknown = fmt.Sprintf("%s[!] Unknown Hash Format!%s\n", red, rst)
	return unknown
}
